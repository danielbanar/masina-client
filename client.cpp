...
int main()
{
    ...
    {
        usleep(1000);
        static uint16_t crsfChannels[16] = {992, 992, 1716, 992, 191, 191, 191, 191, 997, 997, 997, 997, 0, 0, 1811, 1811};
            ...
            if (bytesRead == -1)
            {
                if (errno == EWOULDBLOCK || errno == EAGAIN)
                {
                    ...
                    else if (elapsedTimeValid >= gConfig.stabTimeout)
                    {
                        ...
                        crsfChannels[0] = CRSF_CHANNEL_VALUE_MID;         // ROLL
                        crsfChannels[1] = CRSF_CHANNEL_VALUE_MID;         // PITCH
                        crsfChannels[2] = US_TO_CRSF(gConfig.hoverValue); // THROTTLE
                        crsfChannels[3] = CRSF_CHANNEL_VALUE_MID;         // YAW
                        crsfChannels[5] = CRSF_CHANNEL_VALUE_MIN;         // ANGLE MODE MODE
                        if (gConfig.debug)
                            printf("STAB TIMEOUT!\n");
                    }
                    ...
                    if (elapsedTime > 10)
                    {
                        ...
                        crsfPayload[1] = 0x18;
                        crsfPayload[2] = 0x16;
                        crsfPayload[3] = (crsfChannels[0] & 0xFF);
                        crsfPayload[4] = ((crsfChannels[0] >> 8) & 0x07) | ((crsfChannels[1] & 0x1F) << 3);
                        crsfPayload[5] = ((crsfChannels[1] >> 5) & 0x3F) | ((crsfChannels[2] & 0x03) << 6);
                        crsfPayload[6] = ((crsfChannels[2] >> 2) & 0xFF);
                        crsfPayload[7] = ((crsfChannels[2] >> 10) & 0x01) | ((crsfChannels[3] & 0x7F) << 1);
                        crsfPayload[8] = ((crsfChannels[3] >> 7) & 0x0F) | ((crsfChannels[4] & 0x0F) << 4);
                        crsfPayload[9] = ((crsfChannels[4] >> 4) & 0x7F) | ((crsfChannels[5] & 0x01) << 7);
                        crsfPayload[10] = ((crsfChannels[5] >> 1) & 0xFF);
                        crsfPayload[11] = ((crsfChannels[5] >> 9) & 0x03) | ((crsfChannels[6] & 0x3F) << 2);
                        crsfPayload[12] = ((crsfChannels[6] >> 6) & 0x1F) | ((crsfChannels[7] & 0x07) << 5);
                        crsfPayload[13] = ((crsfChannels[7] >> 3) & 0xFF);
                        crsfPayload[14] = (crsfChannels[8] & 0xFF);
                        crsfPayload[15] = ((crsfChannels[8] >> 8) & 0x07) | ((crsfChannels[9] & 0x1F) << 3);
                        crsfPayload[16] = ((crsfChannels[9] >> 5) & 0x3F) | ((crsfChannels[10] & 0x03) << 6);
                        crsfPayload[17] = ((crsfChannels[10] >> 2) & 0xFF);
                        crsfPayload[18] = ((crsfChannels[10] >> 10) & 0x01) | ((crsfChannels[11] & 0x7F) << 1);
                        crsfPayload[19] = ((crsfChannels[11] >> 7) & 0x0F) | ((crsfChannels[12] & 0x0F) << 4);
                        crsfPayload[20] = ((crsfChannels[12] >> 4) & 0x7F) | ((crsfChannels[13] & 0x01) << 7);
                        crsfPayload[21] = ((crsfChannels[13] >> 1) & 0xFF);
                        crsfPayload[22] = ((crsfChannels[13] >> 9) & 0x03) | ((crsfChannels[14] & 0x3F) << 2);
                        crsfPayload[23] = ((crsfChannels[14] >> 6) & 0x1F) | ((crsfChannels[15] & 0x07) << 5);
                        crsfPayload[24] = ((crsfChannels[15] >> 3) & 0xFF);
                ...
                if (std::regex_search(rxBuffer, matches, regexPattern))
                {
                    ...
                    if (lastN < N) // In order
                    {
                        ...
                        for (size_t i = 0; i < payloadLength - 1; i++)
                            crsfChannels[i] = US_TO_CRSF(payload[i + 1]);

                        fsMode = payload[10] == 2000;
